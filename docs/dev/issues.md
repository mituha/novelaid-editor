...
リンクでファイルを開く際の開くビューの種類が時々おかしい？
openDocumentの実装部分を確認する。
特にpreviewから開いた場合の処理は想定と違う実装の可能性がある。
基本的に元のビューと同じビューでのリンクによる遷移を想定しています。
previewは元のドキュメントを必要とします。そのため、previewを開く前にeditorを開く必要があります。
openDocumentの内部で再帰的に事前に開く必要があります。
開くタブのsideも考慮する必要があります。
preview前に開くeditorはleftになる。なお、previewは呼び出し元のsideと同じsideで開く。



元のビューを閉じて仮想的に同じビューで開いたように見せるかは検討。
基本的には元のビューを閉じた状態で開くのが良いかも？
同一階層の遷移のみとし、異なる階層への遷移は新しいタブで開く(元のタブを閉じない)
マウスの中央ボタンでクリック時は新しいタブで開く(元のタブを閉じない)
...
プロジェクトを開いた時のチャットビューとかもEditor側になっていたり微妙におかしい。
