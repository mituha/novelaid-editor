...
### ファイル検索からドキュメントを開いた場合の機能がなくなってる？

検索されたファイルの位置を開く機能、および、エディター内の検索ボックスを開く機能。
処理を汎用にした際になくなった？
...
### ファイル名変更時、旧ファイルと新ファイルの２つのファイルができる

エディター上のFileNameHeader名前変更した時、旧ファイルが残ったままになる(実際には旧ファイル名の保存で再作成)。
DocumentArea.tsx の onBlur で保存しているが、名前変更時、この処理が呼び出されているため、名前変更前のpathで再保存(作成)されてしまう。
まず、このonBlurが呼び出されている原因について調査が必要
