...
リンクでドキュメントを開いた際のドキュメントの表示、タイトル部分がおかしい。
`../dir/file.txt`のリンクの場合に、`./file`のようなタイトル表示になっていた。
ファイルのパスを標準的でない解決方法を行っているのではないか？
タイトルの取得も標準的なファイルタイトル取得メソッドがあればそれを使用する。
現在の内部でのドキュメントパスの扱いがどうなっているかを確認し、一旦共通で正規化するメソッドを通すようにする。
Documentsで管理されるファイルパスはキーとなるため、物理的なファイルに対し一意になるように正規化されている必要がある。
あちこちで個別荷処理されている場合、今後のプラグインからの利用を考慮してまとめておく。
  タイトル取得やパスの取得、解決メソッド。
なお、現在、`preview://`のようなURLを付加している部分についてはDocumentTypeやDocumentViewTypeの導入により区別可能となっていると考えられ、prefixの付加自体不要になっていないかも検討してください。
