...
複数ファイルを開いた時に任意のタブ選択ができない。
多すぎて隠れるタブがある。
タブのリスト表示かスクロール的なボタンの追加？
そもそも、大量にタブに表示しない前提が良い。
開いているタブのリストをファイルエクスプローラーに別途表示でも良さそう。
この形式をとればエクスプローラーからドラッグの機能等が使える。

### `chat`ドキュメントの表示が正しく行われていない。
ドキュメントタイプに基づき、チャット用の表示を行う。
