...
```novelaid-radar
---
titleFontSize: 24 # 24px -> config:themeVariables:fontSize
fontSize: 20 # 20 -> config:themeVariables:radar:axisLabelFontSize
color: "#FF0000"  # -> config:themeVariables:cScale0
title: "ステータス"
名前: "わん太"
config:
  radar:
  ...
  themeVariables:
    ...
    radar:
      curveOpacity: 0
showLegend: false # コードブロック要素。デフォルトをfalse
---
力: 9
魔力: 10
精神力: 10
...
config:
  ...
  theme: dark
  themeVariables:
    fontSize: 20px  # px指定が必要
    cScale0: "#FF0000"
    cScale1: "#00FF00"
    cScale2: "#0000FF"
    radar:
      axisLabelFontSize: 20 # px指定が不可
      curveOpacity: 0.5
title: "ステータス"
---
radar-beta
...
BUG?
現状、fontSizeはpx指定が必須なのに対して、axisLabelFontSizeは数値のみ指定可能。整合性が取れていない。
